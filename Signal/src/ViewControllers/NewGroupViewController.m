...
@interface NewGroupViewController () <UIImagePickerControllerDelegate, UITextFieldDelegate>

@property (nonatomic, readonly) TSGroupThread *thread;
@property (nonatomic, readonly) OWSMessageSender *messageSender;
@property (nonatomic, readonly) OWSContactsManager *contactsManager;

@property (nonatomic, nullable) NSArray<Contact *> *allContacts;
@property (nonatomic, nullable) NSArray<ContactAccount *> *allContactAccounts;
...
- (void)updateTableContents
{
    ...
    if (self.memberRecipientIds.count > 0) {
        ...
        for (NSString *recipientId in [self.memberRecipientIds sortedArrayUsingSelector:@selector(compare:)]) {
            [membersSection addItem:[OWSTableItem itemWithCustomCellBlock:^{
                ...
                ContactAccount *contactAccount = weakSelf.contactAccountMap[recipientId];
                BOOL isPreviousMember = [weakSelf.previousMemberRecipientIds containsObject:recipientId];
                BOOL isCurrentMember = YES;
                BOOL isBlocked = (contactAccount ? [weakSelf isContactBlocked:contactAccount.contact]
                                                 : [weakSelf isRecipientIdBlocked:recipientId]);
                if ((weakSelf.thread && isPreviousMember) || (!weakSelf.thread && isCurrentMember)) {
                ...
                } else if (weakSelf.thread && isCurrentMember) {
                ...
                if (contactAccount) {
                    ...
                    [cell configureWithContact:contactAccount.contact contactsManager:weakSelf.contactsManager];
                } else {
                    [cell configureWithRecipientId:recipientId contactsManager:weakSelf.contactsManager];
                }
    ...
    if (self.allContactAccounts.count > 0) {
        for (ContactAccount *contactAccount in self.allContactAccounts) {
            [contactAccountSection
                addItem:
                    [OWSTableItem
                        itemWithCustomCellBlock:^{
                            ContactTableViewCell *cell = [ContactTableViewCell new];
                            BOOL isBlocked = [weakSelf isContactBlocked:contactAccount.contact];
                            if (isBlocked) {
                                cell.accessoryMessage = NSLocalizedString(
                                    @"CONTACT_CELL_IS_BLOCKED", @"An indicator that a contact has been blocked.");
                            } else {
